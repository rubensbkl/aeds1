...
bool existe_saida(char **labirinto, bool **visitado, int num_linhas, int num_cols, int i, int j)
{
	// Encontrou a saida?
	...
	else if(labirinto[i][j] != '*') 
	{
		...
		// Olha pra baixo
		if((achou == false) && (i < (num_linhas - 1)) && (visitado[i + 1][j] == false))
		{
			achou = existe_saida(labirinto, visitado, num_linhas, num_cols, i + 1, j);
		}
		// Olha pra esquerda
		...
		// Olha pra direita
		if((achou == false) && (j < (num_cols - 1)) && (visitado[i][j + 1] == false))
		{
			achou = existe_saida(labirinto, visitado, num_linhas, num_cols, i, j + 1);
		}

		visitado[i][j] = false; // Desativa pois outro caminho pode precisar dessa posicao
